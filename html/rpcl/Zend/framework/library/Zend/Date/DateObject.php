 ...
 * @version    $Id: DateObjectNew1.php 23775 2011-03-01 17:25:24Z ralph $
 ...
 * @category   Zend
 * @package    Zend_Date
 * @subpackage Zend_Date_DateObjectNew1
 * @copyright  Copyright (c) 2005-2011 Zend Technologies USA Inc. (http://www.zend.com)
 * @license    http://framework.zend.com/license/new-bsd     New BSD License
 */
abstract class Zend_Date_DateObjectNew1 {
    ...
    protected function mktime($hour, $minute, $second, $month, $day, $year, $gmt = false)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('Zend_DateObjectNew1_mkTime_' . $this->_offset . '_' . $year.$month.$day.'_'.$hour.$minute.$second . '_'.(int)$gmt, '-','_');
    ...
    protected function date($format, $timestamp = null, $gmt = false)
    {
        ...
        $origstamp = $timestamp;
        if (isset(self::$_cache)) {
            $idstamp = strtr('Zend_DateObjectNew1_date_' . $this->_offset . '_'. $timestamp . '_'.(int)$gmt, '-','_');
    ...
    protected function getDateParts($timestamp = null, $fast = null)
    {
        ...
        if (isset(self::$_cache)) {
            $id = strtr('Zend_DateObjectNew1_getDateParts_' . $timestamp.'_'.(int)$fast, '-','_');
            if ($result = self::$_cache->load($id)) {
                return unserialize($result);
            }
        }
     ...
     * @return Zend_Date_DateObjectNew1 Provides fluent interface
     * @throws Zend_Date_Exception
     */
