...
class Zend_Search_Lucene_Index_SegmentInfo implements Zend_Search_Lucene_Index_TermsStream_Interface
{
    ...
    public function __construct(Zend_Search_Lucene_Storage_Directory $directory, $name, $docCount, $delGen = 0, $docStoreOptions = null, $hasSingleNormFile = false, $isCompound = null)
    {
        ...
        if ($docStoreOptions !== null) {
            ...
            if ($docStoreOptions['isCompound']) {
                $cfxFile       = $this->_directory->getFileObjectNew1($docStoreOptions['segment'] . '.cfx');
        ...
        if ($isCompound !== null) {
            ...
            try {
                ...
                $this->_directory->getFileObjectNew1($name . '.cfs');
        ...
        if ($this->_isCompound) {
            $cfsFile = $this->_directory->getFileObjectNew1($name . '.cfs');
    ...
    private function _loadPre21DelFile()
    {
        ...
        try {
            ...
            $delFile = $this->_directory->getFileObjectNew1($this->_name . '.del');
    ...
    {
        $delFile = $this->_directory->getFileObjectNew1($this->_name . '_' . base_convert($this->_delGen, 10, 36) . '.del');
    ...
    public function openCompoundFile($extension, $shareHandler = true)
    {
        if (($extension == '.fdx'  || $extension == '.fdt')  &&  $this->_usesSharedDocStore) {
            ...
            if (!$this->_sharedDocStoreOptions['isCompound']) {
                $fdxFile = $this->_directory->getFileObjectNew1($fdxFName, $shareHandler);
                ...
                if ($extension == '.fdx') {
                    ...
                    $fdtFile = $this->_directory->getFileObjectNew1($fdtFName, $shareHandler);
            ...
            $cfxFile = $this->_directory->getFileObjectNew1($this->_sharedDocStoreOptions['segment'] . '.cfx', $shareHandler);
        ...
        if (!$this->_isCompound) {
            return $this->_directory->getFileObjectNew1($filename, $shareHandler);
        }
        ...
        $file = $this->_directory->getFileObjectNew1($this->_name . '.cfs', $shareHandler);
    ...
    private function _loadDictionaryIndex()
    {
        ...
        if ($this->_directory->fileExists($this->_name . '.sti')) {
            ...
            $stiFile = $this->_directory->getFileObjectNew1($this->_name . '.sti');
