...
class Zend_Search_Lucene_LockManager
{
    ...
    {
        $lock = $lockDirectory->getFileObjectNew1(self::WRITE_LOCK_FILE);
        $lock->unlock();
    }
    ...
    {
        $lock = $lockDirectory->getFileObjectNew1(self::READ_LOCK_PROCESSING_LOCK_FILE);
        $lock->unlock();
    }
    ...
    {
        $lock = $lockDirectory->getFileObjectNew1(self::READ_LOCK_FILE);
        $lock->unlock();
    }
    ...
    {
        $lock = $lockDirectory->getFileObjectNew1(self::OPTIMIZATION_LOCK_FILE);
        $lock->unlock();
    }
