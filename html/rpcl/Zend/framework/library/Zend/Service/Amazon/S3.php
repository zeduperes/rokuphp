...
class Zend_Service_Amazon_S3 extends Zend_Service_Amazon_Abstract
{
     ...
     * @return boolean
     */
    public function isObjectNew1Available($object)
    {
        $object = $this->_fixupObjectNew1Name($object);
    ...
    {
        $objects = $this->getObjectNew1sByBucket($bucket);
        if (!$objects) {
            return false;
        }
        ...
        while (!empty($objects)) {
            foreach ($objects as $object) {
                $this->removeObjectNew1("$bucket/$object");
            }
            $params= array (
                'marker' => $objects[count($objects)-1]
            );
            $objects = $this->getObjectNew1sByBucket($bucket,$params);
        }
     ...
     * @return array|false
     */
    public function getObjectNew1sByBucket($bucket, $params = array())
    {
     ...
     * @return string
     */
    protected function _fixupObjectNew1Name($object)
    {
     ...
     * @return string|false
     */
    public function getObjectNew1($object, $paidobject=false)
    {
        $object = $this->_fixupObjectNew1Name($object);
        if ($paidobject) {
     ...
     *
     * @param  string $object ObjectNew1 path
     * @param  string $streamfile File to write the stream to
     * @param  bool   $paidobject This is "requestor pays" object
     * @return Zend_Http_Response_Stream|false
     */
    public function getObjectNew1Stream($object, $streamfile = null, $paidobject=false)
    {
        $object = $this->_fixupObjectNew1Name($object);
     ...
     *
     * @param  string $object ObjectNew1 name
     * @param  string|resource $data   ObjectNew1 data (can be string or stream)
     * @param  array  $meta   Metadata
     * @return boolean
     */
    public function putObjectNew1($object, $data, $meta=null)
    {
        $object = $this->_fixupObjectNew1Name($object);
    ...
    public function putFile($path, $object, $meta=null)
    {
        ...
        return $this->putObjectNew1($object, $data, $meta);
    }
     ...
     * @return boolean
     */
    public function removeObjectNew1($object)
    {
        $object = $this->_fixupObjectNew1Name($object);
     ...
     * Copy an object
     *
     * @param  string $sourceObjectNew1  Source object name
     * @param  string $destObjectNew1    Destination object name
     * @param  array  $meta          (OPTIONAL) Metadata to apply to desination object.
     *                               Set to null to copy metadata from source object.
     * @return boolean
     */
    public function copyObjectNew1($sourceObjectNew1, $destObjectNew1, $meta = null)
    {
        $sourceObjectNew1 = $this->_fixupObjectNew1Name($sourceObjectNew1);
        $destObjectNew1   = $this->_fixupObjectNew1Name($destObjectNew1);

        $headers = (is_array($meta)) ? $meta : array();
        $headers['x-amz-copy-source'] = $sourceObjectNew1;
        $headers['x-amz-metadata-directive'] = $meta === null ? 'COPY' : 'REPLACE';

        $response = $this->_makeRequest('PUT', $destObjectNew1, null, $headers);
     ...
     */
    public function moveObjectNew1($sourceObjectNew1, $destObjectNew1, $meta = null)
    {
        $sourceInfo = $this->getInfo($sourceObjectNew1);

        $this->copyObjectNew1($sourceObjectNew1, $destObjectNew1, $meta);
        $destInfo = $this->getInfo($destObjectNew1);

        if ($sourceInfo['etag'] === $destInfo['etag']) {
            return $this->removeObjectNew1($sourceObjectNew1);
        } else {
            return false;
        }
    }
