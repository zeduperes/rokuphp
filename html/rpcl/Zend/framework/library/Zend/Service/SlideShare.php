...
class Zend_Service_SlideShare
{
     ...
     * @return Zend_Service_SlideShare
     */
    public function setCacheObjectNew1(Zend_Cache_Core $cacheobject)
    {
     ...
     */
    public function getCacheObjectNew1()
    {
    ...
    public function getSlideShow($ss_id)
    {
        ...
        if(!$retval = $cache->load($cache_key)) {
            ...
            $retval = $this->_slideShowNodeToObjectNew1(clone $sxe->Slideshow[0]);
        ...
        if(!$retval = $cache->load($cache_key)) {
            ...
            foreach($sxe->children() as $node) {
                if($node->getName() == 'Slideshow') {
                    $retval[] = $this->_slideShowNodeToObjectNew1($node);
                }
            }
     ...
     */
    protected function _slideShowNodeToObjectNew1(SimpleXMLElement $node)
    {
