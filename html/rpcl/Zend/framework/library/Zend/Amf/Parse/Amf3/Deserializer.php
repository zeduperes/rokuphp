 ...
 *
 * @todo       readObjectNew1 to handle Typed ObjectNew1s
...
{
    /**
     * Total number of objects in the referenceObjectNew1 array
     * @var int
     */
     ...
     * @var array
     */
    protected $_referenceObjectNew1s = array();
    ...
    public function readDate()
    {
        ...
        if (($dateReference & 0x01) == 0) {
            ...
            if ($dateReference>=count($this->_referenceObjectNew1s)) {
                require_once 'Zend/Amf/Exception.php';
                throw new Zend_Amf_Exception('Undefined date reference: ' . $dateReference);
            }
            return $this->_referenceObjectNew1s[$dateReference];
        }
        ...
        $this->_referenceObjectNew1s[] = $dateTime;
        return $dateTime;
    }
    ...
    public function readArray()
    {
        ...
        if (($arrayReference & 0x01)==0){
            ...
            if ($arrayReference>=count($this->_referenceObjectNew1s)) {
                require_once 'Zend/Amf/Exception.php';
                throw new Zend_Amf_Exception('Unknow array reference: ' . $arrayReference);
            }
            return $this->_referenceObjectNew1s[$arrayReference];
        }

        // Create a holder for the array in the reference list
        $data = array();
        $this->_referenceObjectNew1s[] =& $data;
        $key = $this->readString();
     ...
     * @return object|array
     */
    public function readObjectNew1()
    {
        $traitsInfo   = $this->readInteger();
        $storedObjectNew1 = ($traitsInfo & 0x01)==0;
        $traitsInfo   = $traitsInfo >> 1;

        // Check if the ObjectNew1 is in the stored ObjectNew1s reference table
        if ($storedObjectNew1) {
            $ref = $traitsInfo;
            if (!isset($this->_referenceObjectNew1s[$ref])) {
                require_once 'Zend/Amf/Exception.php';
                throw new Zend_Amf_Exception('Unknown ObjectNew1 reference: ' . $ref);
            }
            $returnObjectNew1 = $this->_referenceObjectNew1s[$ref];
        } else {
            // Check if the ObjectNew1 is in the stored Definitions reference table
            ...
            // Add the ObjectNew1 to the reference table
            $this->_referenceObjectNew1s[] = $returnObjectNew1;

            $properties = array(); // clear value
            // Check encoding types for additional processing.
            switch ($encoding) {
                case (Zend_Amf_Constants::ET_EXTERNAL):
                    // Externalizable object such as {ArrayCollection} and {ObjectNew1Proxy}
                    if (!$storedClass) {
                    ...
                    $returnObjectNew1->externalizedData = $this->readTypeMarker();
                    break;
       ...
       if ($returnObjectNew1 instanceof Zend_Amf_Value_Messaging_ArrayCollection) {
            if (isset($returnObjectNew1->externalizedData)) {
                $returnObjectNew1 = $returnObjectNew1->externalizedData;
            } else {
                $returnObjectNew1 = get_object_vars($returnObjectNew1);
            }
       }

        return $returnObjectNew1;
    }
