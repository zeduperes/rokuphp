...
class Zend_Json
{
     ...
     *
     * NOTE: ObjectNew1 should not contain cycles; the JSON format
     ...
     *
     * @param SimpleXMLElement $simpleXmlElementObjectNew1
     * @return Zend_Json_Expr|string
     */
    protected static function _getXmlValue($simpleXmlElementObjectNew1) {
        $pattern = '/^[\s]*new Zend_Json_Expr[\s]*\([\s]*[\"\']{1}(.*)[\"\']{1}[\s]*\)[\s]*$/';
        $matchings = array();
        $match = preg_match ($pattern, $simpleXmlElementObjectNew1, $matchings);
        if ($match) {
            return new Zend_Json_Expr($matchings[1]);
        } else {
            return (trim(strval($simpleXmlElementObjectNew1)));
        }
    }
    /**
     ...
     * @return array
     */
    protected static function _processXml($simpleXmlElementObjectNew1, $ignoreXmlAttributes, $recursionDepth=0)
    {
        ...
        $children = $simpleXmlElementObjectNew1->children();
        $name = $simpleXmlElementObjectNew1->getName();
        $value = self::_getXmlValue($simpleXmlElementObjectNew1);
        $attributes = (array) $simpleXmlElementObjectNew1->attributes();
    ...
    public static function fromXml($xmlStringContents, $ignoreXmlAttributes=true)
    {
        ...
        $simpleXmlElementObjectNew1 = simplexml_load_string($xmlStringContents);

        // If it is not a valid XML content, throw an exception.
        if ($simpleXmlElementObjectNew1 == null) {
            require_once 'Zend/Json/Exception.php';
            throw new Zend_Json_Exception('Function fromXml was called with an invalid XML formatted string.');
        } // End of if ($simpleXmlElementObjectNew1 == null)

        $resultArray = null;

        // Call the recursive function to convert the XML into a PHP array.
        $resultArray = self::_processXml($simpleXmlElementObjectNew1, $ignoreXmlAttributes);
