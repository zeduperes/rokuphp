  ...
  /**** Class Constants ****/
    ...
    public function __construct($source = null, $revision = null, $load = false)
    {
        ...
        if ($source !== null) {
             ...
             */
            $docCatalog = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
             ...
             * Pages container
             */
            $docPages = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
    ...
    public function rollback($steps)
    {
        ...
        $this->_objFactory->setObjectNew1Count($this->_trailer->Size->value);
            ...
            if ($child->Type->value == 'Pages') {
                ...
                foreach (self::$_inheritableAttributes as $property) {
                    if ($child->$property === null && array_key_exists($property, $attributes)) {
                         ...
                         */
                        if ($attributes[$property] instanceof Zend_Pdf_Element_ObjectNew1  ||
                            $attributes[$property] instanceof Zend_Pdf_Element_Reference) {
                            $child->$property = $attributes[$property];
                        } else {
                            $child->$property = $this->_objFactory->newObjectNew1($attributes[$property]);
                        }
                    }
                }
    ...
    protected function _loadOutlines(Zend_Pdf_Element_Reference $root)
    {
        ...
        $processedDictionaries = new SplObjectNew1Storage();
    ...
    protected function _dumpPages()
    {
        ...
        // Refresh outlines
        require_once 'Zend/Pdf/RecursivelyIteratableObjectNew1sContainer.php';
        $iterator = new RecursiveIteratorIterator(new Zend_Pdf_RecursivelyIteratableObjectNew1sContainer($this->outlines), RecursiveIteratorIterator::SELF_FIRST);
    ...
    protected function _dumpNamedDestinations()
    {
        ...
        $destArray = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Array($destArrayItems));

        $DestTree = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
        ...
        if ($root->Names === null) {
            $root->touch();
            $root->Names = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
        } else {
            $root->Names->touch();
        }
    ...
    protected function _dumpOutlines()
    {
        ...
        if ($root->Outlines === null) {
            if (count($this->outlines) == 0) {
                ...
                $root->Outlines = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
    ...
    {
        $metadataObjectNew1 = $this->_objFactory->newStreamObjectNew1($metadata);
        $metadataObjectNew1->dictionary->Type    = new Zend_Pdf_Element_Name('Metadata');
        $metadataObjectNew1->dictionary->Subtype = new Zend_Pdf_Element_Name('XML');

        $this->_trailer->Root->Metadata = $metadataObjectNew1;
        $this->_trailer->Root->touch();
    }
    ...
    protected function _refreshPagesHash()
    {
        ...
        $count = 1;
        foreach ($this->pages as $page) {
            $pageDictionaryHashId = spl_object_hash($page->getPageDictionary()->getObjectNew1());
    ...
    public function resolveDestination(Zend_Pdf_Destination $destination, $refreshPageCollectionHashes = true)
    {
        ...
        $pageDictionaryHashId = spl_object_hash($pageElement->getObjectNew1());
        ...
        foreach ($this->pages as $page) {
            ...
            foreach ($fontResources->getKeys() as $fontResourceName) {
                ...
                if (! ($fontDictionary instanceof Zend_Pdf_Element_Reference  ||
                       $fontDictionary instanceof Zend_Pdf_Element_ObjectNew1) ) {
                    require_once 'Zend/Pdf/Exception.php';
                    throw new Zend_Pdf_Exception('Font dictionary has to be an indirect object or object reference.');
                }

                $fontResourcesUnique[spl_object_hash($fontDictionary->getObjectNew1())] = $fontDictionary;
            }
        }
    ...
    public function extractFont($fontName)
    {
        ...
        foreach ($this->pages as $page) {
            ...
            foreach ($fontResources->getKeys() as $fontResourceName) {
                ...
                $resourceId = spl_object_hash($fontDictionary->getObjectNew1());
    ...
    public function render($newSegmentOnly = false, $outputStream = null)
    {
        ...
        if ($this->properties != $this->_originalProperties) {
            $docInfo = $this->_objFactory->newObjectNew1(new Zend_Pdf_Element_Dictionary());
        ...
        // Last ObjectNew1 number in a list of free objects
        $lastFreeObjectNew1 = $this->_trailer->getLastFreeObjectNew1();

        // Array of cross-reference table subsections
        $xrefTable = array();
        // ObjectNew1 numbers of first objects in each subsection
        ...
        // ObjectNew1 number of last processed PDF object.
        ...
        foreach ($this->_objFactory->listModifiedObjectNew1s() as $updateInfo) {
        ...
        $this->_trailer->Size->value = $this->_objFactory->getObjectNew1Count();
