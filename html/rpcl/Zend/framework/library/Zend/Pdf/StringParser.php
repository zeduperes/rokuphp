...
//        }
    ...
    public function readElement($nextLexeme = null)
    {
         ...
         * If readElement() is used not by Zend_Pdf_StringParser::getObjectNew1() method, then we should not care
     ...
     */
    public function getObjectNew1($offset, Zend_Pdf_Element_Reference_Context $context)
    {
        if ($offset === null ) {
            return new Zend_Pdf_Element_Null();
        }

        // Save current offset to make getObjectNew1() reentrant
        ...
        if (!ctype_digit($objNum)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(sprintf('PDF file syntax error. Offset - 0x%X. ObjectNew1 number expected.', $this->offset - strlen($objNum)));
        }

        $genNum = $this->readLexeme();
        if (!ctype_digit($genNum)) {
            require_once 'Zend/Pdf/Exception.php';
            throw new Zend_Pdf_Exception(sprintf('PDF file syntax error. Offset - 0x%X. ObjectNew1 generation number expected.', $this->offset - strlen($genNum)));
        }
        ...
        if( $nextLexeme == 'endobj' ) {
            /**
             * ObjectNew1 is not generated by factory (thus it's not marked as modified object).
             * But factory is assigned to the obect.
             */
            $obj = new Zend_Pdf_Element_ObjectNew1($objValue, (int)$objNum, (int)$genNum, $this->_objFactory->resolve());

            foreach ($this->_elements as $element) {
                $element->setParentObjectNew1($obj);
            }
        ...
        $obj = new Zend_Pdf_Element_ObjectNew1_Stream(substr($this->data,
                                                         $dataOffset,
                                                         $streamLength),
                                                  (int)$objNum,
                                                  (int)$genNum,
