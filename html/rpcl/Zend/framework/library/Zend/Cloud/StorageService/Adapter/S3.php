    ...
    implements Zend_Cloud_StorageService_Adapter
{
    ...
    public function fetchItem($path, $options = array())
    {
        ...
        try {
            if (!empty($options[self::FETCH_STREAM])) {
                return $this->_s3->getObjectNew1Stream($fullPath, $options[self::FETCH_STREAM]);
            } else {
                return $this->_s3->getObjectNew1($fullPath);
            }
        } catch (Zend_Service_Amazon_S3_Exception  $e) {
    ...
    {
        try {
            $this->_s3->removeObjectNew1($this->_getFullPath($path, $options));
        } catch (Zend_Service_Amazon_S3_Exception  $e) {
    ...
    public function moveItem($sourcePath, $destinationPath, $options = array())
    {
        try {
            ...
            return $this->_s3->moveObjectNew1(
                $fullSourcePath,
                $fullDestPath,
                empty($options[self::METADATA]) ? null : $options[self::METADATA]
            );
    ...
    {
        try {
            // TODO Support 'prefix' parameter for Zend_Service_Amazon_S3::getObjectNew1sByBucket()
            return $this->_s3->getObjectNew1sByBucket($this->_defaultBucketName);
        } catch (Zend_Service_Amazon_S3_Exception  $e) {
