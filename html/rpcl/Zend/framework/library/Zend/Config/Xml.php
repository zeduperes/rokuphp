...
class Zend_Config_Xml extends Zend_Config
{
     ...
     * a SimpleXMLElement.
     *
     * @param  SimpleXMLElement $xmlObjectNew1 Convert a SimpleXMLElement into an array
     * @return array|string
     */
    protected function _toArray(SimpleXMLElement $xmlObjectNew1)
    {
        $config       = array();
        $nsAttributes = $xmlObjectNew1->attributes(self::XML_NAMESPACE);

        // Search for parent node values
        if (count($xmlObjectNew1->attributes()) > 0) {
            foreach ($xmlObjectNew1->attributes() as $key => $value) {
                if ($key === 'extends') {
                    continue;
                }
        ...
        if (count($xmlObjectNew1->children(self::XML_NAMESPACE)) > 0) {
            if (count($xmlObjectNew1->children()) > 0) {
                require_once 'Zend/Config/Exception.php';
                throw new Zend_Config_Exception("A node with a 'const' childnode may not have any other children");
            }

            $dom                 = dom_import_simplexml($xmlObjectNew1);
        ...
        // Search for children
        if (count($xmlObjectNew1->children()) > 0) {
            foreach ($xmlObjectNew1->children() as $key => $value) {
        ...
        } else if (!isset($xmlObjectNew1['extends']) && !isset($nsAttributes['extends']) && (count($config) === 0)) {
            // ObjectNew1 has no children nor attributes and doesn't use the extends
            // attribute: it's a string
            $config = (string) $xmlObjectNew1;
        }
