...
class Zend_Wildfire_Plugin_FirePhp implements Zend_Wildfire_Plugin_Interface
{
    ...
    protected $_options = array(
        ...
        'maxObjectNew1Depth' => 10, /* The maximum depth to traverse objects when encoding */
     ...
     * @return void
     */
    public function setObjectNew1Filter($class, $filter) {
    ...
    public static function send($var, $label=null, $style=null, $options=array())
    {
        ...
        if ($meta['Type']==self::TRACE) {
            ...
            $var = array('Class'=>$trace[0]['class'],
                         ...
                         'Args'=>isset($trace[0]['args'])?$firephp->_encodeObjectNew1($trace[0]['args']):'',
    ...
    protected function _recordMessage($structure, $data, $skipEncode=false)
    {
        switch($structure) {
            ...
            case self::STRUCTURE_URI_DUMP:
                ...
                $value = $data['data'];
                if (!$skipEncode) {
                  $value = $this->_encodeObjectNew1($data['data']);
                }
    ...
    /**
     * Encodes a table by encoding each row and column with _encodeObjectNew1()
     *
      ...
      for ($i=0 ; $i<count($table) ; $i++) {
          if (is_array($table[$i])) {
              for ($j=0 ; $j<count($table[$i]) ; $j++) {
                  $table[$i][$j] = $this->_encodeObjectNew1($table[$i][$j]);
              }
          }
        }
      return $table;
    ...
    /**
     * Encodes a trace by encoding all "args" with _encodeObjectNew1()
     *
    ...
    protected function _encodeTrace($trace)
    {
      ...
      for ($i=0 ; $i<sizeof($trace) ; $i++) {
          if (isset($trace[$i]['args'])) {
              $trace[$i]['args'] = $this->_encodeObjectNew1($trace[$i]['args']);
          }
      }
      return $trace;
    }
     ...
     */
    protected function _encodeObjectNew1($object, $objectDepth = 1, $arrayDepth = 1)
    {
        $return = array();
        ...
        if (is_object($object)) {
            ...
            if ($objectDepth > $this->_options['maxObjectNew1Depth']) {
              return '** Max ObjectNew1 Depth ('.$this->_options['maxObjectNew1Depth'].') **';
            }
            ...
            foreach ($properties as $just_name => $property) {
                ...
                if (!(isset($this->_objectFilters[$class])
                    ...
                    if (array_key_exists($raw_name,$members)
                        ...
                        $return[$name] = $this->_encodeObjectNew1($members[$raw_name], $objectDepth + 1, 1);

                    } else {
                        if (method_exists($property,'setAccessible')) {
                            $property->setAccessible(true);
                            $return[$name] = $this->_encodeObjectNew1($property->getValue($object), $objectDepth + 1, 1);
                        } else
                        if ($property->isPublic()) {
                            $return[$name] = $this->_encodeObjectNew1($property->getValue($object), $objectDepth + 1, 1);
                        } else {
            ...
            foreach($members as $just_name => $value) {
                ...
                if (!isset($properties[$name])) {
                    ...
                    if (!(isset($this->objectFilters[$class])
                      ...
                      $return[$name] = $this->_encodeObjectNew1($value, $objectDepth + 1, 1);
                    } else {
                      $return[$name] = '** Excluded by Filter **';
                    }
                }
            ...
            foreach ($object as $key => $val) {
              ...
              $return[$key] = $this->_encodeObjectNew1($val, 1, $arrayDepth + 1);
            }
        } else {
            return $object;
        }
