...
//EndSkipAIO
	...
	*/
	...
	/*
		...
		$sFunction || $objObjectNew1 || $sEvent - (mixed):
...
//EndSkipDebug
	...
	*/
	...
	/*
		Function: registerCallableObjectNew1
		...
		oObjectNew1 - (object, by reference):  The object whose methods will be
			registered.
		...
		deprecated : This function will be removed in future versions.  Please
			use <xajax->register> instead.
	*/
	function registerCallableObjectNew1(&$oObjectNew1)
	{
		$mResult = false;
		
		if (0 > version_compare(PHP_VERSION, '5.0'))
			// for PHP4; using eval because PHP5 will complain it is deprecated
			eval('$mResult = $this->register(XAJAX_CALLABLE_OBJECT, &$oObjectNew1);');
		else
			// for PHP5
			$mResult = $this->register(XAJAX_CALLABLE_OBJECT, $oObjectNew1);
