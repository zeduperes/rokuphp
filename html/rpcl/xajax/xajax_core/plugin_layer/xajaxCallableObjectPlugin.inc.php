<?php
/*
	File: xajaxCallableObjectNew1Plugin.inc.php

	Contains the xajaxCallableObjectNew1Plugin class

	Title: xajaxCallableObjectNew1Plugin class
...
/*
	@package xajax
	@version $Id: xajaxCallableObjectNew1Plugin.inc.php 362 2007-05-29 15:32:24Z calltoconstruct $
...
//SkipAIO
require dirname(__FILE__) . '/support/xajaxCallableObjectNew1.inc.php';
//EndSkipAIO

/*
	Class: xajaxCallableObjectNew1Plugin
*/
class xajaxCallableObjectNew1Plugin extends xajaxRequestPlugin
{
	/*
		Array: aCallableObjectNew1s
	*/
	var $aCallableObjectNew1s;
	...
	*/
	...
	/*
		Function: xajaxCallableObjectNew1Plugin
	*/
	function xajaxCallableObjectNew1Plugin()
	{
		$this->aCallableObjectNew1s = array();
...
//EndSkipDebug
				...
				if (false === is_a($xco, 'xajaxCallableObjectNew1'))
					$xco =& new xajaxCallableObjectNew1($xco);
	...
	*/
	function generateClientScript()
	{
		...
		{
			if (0 < count($this->aCallableObjectNew1s))
			{
				$sCrLf = "\n";
				...
				foreach(array_keys($this->aCallableObjectNew1s) as $sKey)
					$this->aCallableObjectNew1s[$sKey]->generateClientScript($this->sXajaxPrefix);
	...
	*/
	function processRequest()
	{
		...
		foreach (array_keys($this->aCallableObjectNew1s) as $sKey)
		{
			$xco =& $this->aCallableObjectNew1s[$sKey];
...
$objPluginManager->registerPlugin(new xajaxCallableObjectNew1Plugin(), 102);
