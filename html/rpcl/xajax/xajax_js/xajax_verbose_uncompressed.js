...
try {
	...
	*/
	...
	/*
		Function: xajax.debug.verbose.expandObjectNew1
		...
		string - The textual representation of all the first
			level members.
	*/	
	xajax.debug.verbose.expandObjectNew1 = function(obj) {
		var rec = true;
		...
		if ('function' == typeof (obj)) {
			...
			if (true == rec) {
				...
				for (var m in obj) {
					...
					t += ': ';
					try {
						t += xajax.debug.verbose.expandObjectNew1(obj[m], false);
					} catch (e) {
						t += '[n/a]';
					}
					separator = ', ';
	...
	xajax.debug.verbose.makeFunction = function(obj, name) {
		return function() {
			...
			for (var p = 0; p < pLen; ++p) {
				fun += separator;
				fun += xajax.debug.verbose.expandObjectNew1(arguments[p]);
				separator = ',';
			}
			...
			msg += fun;
			msg += ' returns ';
			msg += xajax.debug.verbose.expandObjectNew1(returnValue);
