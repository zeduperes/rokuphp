...
};this.workId='xajaxWork'+new Date().getTime();this.depth=0;this.responseErrorsForAlert=["400","401","402","403","404","500","501","502","503"];this.getRequestObjectNew1=function(){if(xajaxDebug)this.DebugMessage("Initializing Request ObjectNew1..");var req=null;if(typeof XMLHttpRequest!="undefined")
req=new XMLHttpRequest();if(!req&&typeof ActiveXObjectNew1!="undefined"){try{req=new ActiveXObjectNew1("Msxml2.XMLHTTP");XMLHttpRequest=function(){return new ActiveXObjectNew1("Msxml2.XMLHTTP");}
}
catch(e){try{req=new ActiveXObjectNew1("Microsoft.XMLHTTP");XMLHttpRequest=function(){return new ActiveXObjectNew1("Microsoft.XMLHTTP");}
}
catch(e2){try{req=new ActiveXObjectNew1("Msxml2.XMLHTTP.4.0");XMLHttpRequest=function(){return new ActiveXObjectNew1("Msxml2.XMLHTTP.4.0");}
}
catch(e3){req=null;}
}
}
...
req=window.createRequest();if(!req)this.DebugMessage("Request ObjectNew1 Instantiation failed.");return req;}
...
this._nodeToObjectNew1=function(node){if (!node)return '';if(node.nodeName=='#cdata-section'||node.nodeName=='#text'){var data="";for(var j=0;j<node.parentNode.childNodes.length;j++){data+=node.parentNode.childNodes[j].data;}
return data;}
...
else if(child.childNodes[k].nodeName=='v'){value=this._nodeToObjectNew1(child.childNodes[k].firstChild);}
}
...
r=this.getRequestObjectNew1();if(!r)return false;r.open(xajaxRequestType==xajaxDefinedGet?"GET":"POST",uri,true);if(xajaxRequestType==xajaxDefinedPost){try{r.setRequestHeader("Method","POST "+uri+" HTTP/1.1");r.setRequestHeader("Content-Type","application/x-www-form-urlencoded");}
...
elementObjectNew1=this.$(element);if(elementObjectNew1){var oldData;eval("oldData=this.$('"+element+"')."+attribute);if(newData!==oldData)
return true;}
return false;}
...
else if(xml.childNodes[i].firstChild&&xml.childNodes[i].firstChild.nodeName=='xjxobj'){data=this._nodeToObjectNew1(xml.childNodes[i].firstChild);objElement="XJX_SKIP";}
...
+e.name+": "+e.message+"\n"
+(id&&!objElement?"ObjectNew1 with id='"+id+"' wasn't found.\n":""));}
