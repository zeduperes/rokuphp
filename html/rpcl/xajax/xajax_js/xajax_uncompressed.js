...
function Xajax()
{
	...
	//Get the XMLHttpRequest ObjectNew1
	this.getRequestObjectNew1 = function()
	{
		if (xajaxDebug) this.DebugMessage("Initializing Request ObjectNew1..");
		var req = null;
		if (typeof XMLHttpRequest != "undefined")
			req = new XMLHttpRequest();
		if (!req && typeof ActiveXObjectNew1 != "undefined")
		{
			try
			{
				req=new ActiveXObjectNew1("Msxml2.XMLHTTP");
				XMLHttpRequest = function(){return new ActiveXObjectNew1("Msxml2.XMLHTTP");}
			}
			catch (e)
			{
				try
				{
					req=new ActiveXObjectNew1("Microsoft.XMLHTTP");
					XMLHttpRequest = function(){return new ActiveXObjectNew1("Microsoft.XMLHTTP");}
				}
				catch (e2)
				{
					try {
						req=new ActiveXObjectNew1("Msxml2.XMLHTTP.4.0");
						XMLHttpRequest = function(){return new ActiveXObjectNew1("Msxml2.XMLHTTP.4.0");}
					}
					catch (e3)
					{
						req=null;
		...
		if (!req) this.DebugMessage("Request ObjectNew1 Instantiation failed.");
	...
	this._nodeToObjectNew1 = function(node) {
		if (!node)
			return '';
				...
				if (child.nodeName == 'e') {
					for (var k=0; k<child.childNodes.length; k++) {
						if (child.childNodes[k].nodeName == 'k') {
							...
							value = this._nodeToObjectNew1(child.childNodes[k].firstChild);
						}
					}
	...
	this.willChange = function(element, attribute, newData)
	{
		...
		elementObjectNew1 = this.$(element);
		if (elementObjectNew1) {
			var oldData;		
		...
		for (var i=0; i<xml.childNodes.length; i++)
		{
			...
			if (xml.childNodes[i].nodeName == "cmd")
			{
				...
				if (xml.childNodes[i].childNodes.length > 1 && (xml.childNodes[i].firstChild.nodeName == "#cdata-section" || xml.childNodes[i].firstChild.nodeName == '#text'))
				{
					...
					data = this._nodeToObjectNew1(xml.childNodes[i].firstChild);
					objElement = "XJX_SKIP";
				}
