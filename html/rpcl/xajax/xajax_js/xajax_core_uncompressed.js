...
/*
	...
	<xajax.config.maxObjectNew1Depth> and <xajax.config.maxObjectNew1Size>
*/
...
/*
	Function: xajax.tools._nodeToObjectNew1
...
*/
xajax.tools._nodeToObjectNew1 = function(node) {
	if (null == node)
		return '';
				...
				if ('e' == child.nodeName) {
					...
					while (grandChild) {
						if ('k' == grandChild.nodeName)
							...
							value = xajax.tools._nodeToObjectNew1(grandChild.firstChild);
...
/*
	Function: xajax.tools.getRequestObjectNew1
...
*/
xajax.tools.getRequestObjectNew1 = function() {
	if ('undefined' != typeof XMLHttpRequest) {
		xajax.tools.getRequestObjectNew1 = function() {
			return new XMLHttpRequest();
		}
	} else if ('undefined' != typeof ActiveXObjectNew1) {
		xajax.tools.getRequestObjectNew1 = function() {
			try {
				return new ActiveXObjectNew1('Msxml2.XMLHTTP.4.0');
			} catch (e) {
				xajax.tools.getRequestObjectNew1 = function() {
					try {
						return new ActiveXObjectNew1('Msxml2.XMLHTTP');
					} catch (e2) {
						xajax.tools.getRequestObjectNew1 = function() {
							return new ActiveXObjectNew1('Microsoft.XMLHTTP');
						}
...
/*
	...
	may be an object, which will be deserialized by <xajax._nodeToObjectNew1>
	...
	if (0 < child.childNodes.length) {
		if (1 < child.childNodes.length) {
			...
			if ('xjxobj' == grandChild.nodeName) {
				obj.data = xajax.tools._nodeToObjectNew1(grandChild);
				return;
...
xajax.tools.xml.processFragment = function(xmlNode, seq, oRet, oRequest) {
	...
	while (xmlNode) {
		if ('cmd' == xmlNode.nodeName) {
			...
			oRet = xt._nodeToObjectNew1(xmlNode.firstChild);
		} else if ('debugmsg' == xmlNode.nodeName) {
			// txt = xt._nodeToObjectNew1(xmlNode.firstChild);
		} else 
...
/*
	ObjectNew1: responseSuccessCodes
...
/*
	ObjectNew1: responseErrorsForAlert
...
/*
	ObjectNew1: responseRedirectCodes
...
xajax.initializeRequest = function(oRequest) {
	...
	oRequest.set('maxObjectNew1Depth', xc.maxObjectNew1Depth);
	oRequest.set('maxObjectNew1Size', xc.maxObjectNew1Size);
		...
		while (i < iLen) {
			...
			if ('object' == typeof oVal && null != oVal) {
				try {
					var oGuard = {};
					oGuard.depth = 0;
					oGuard.maxDepth = oRequest.maxObjectNew1Depth;
					oGuard.size = 0;
					oGuard.maxSize = oRequest.maxObjectNew1Size;
...
xajax.prepareRequest = function(oRequest) {
	...
	oRequest.request = xt.getRequestObjectNew1();
