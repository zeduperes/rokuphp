...
function ServerFunctionExecutor(className, connectionInfo, owner)
{
   ...
   * @param requestFilters JSON ObjectNew1 containing pairs of key/value filters to add to the request (filters such as ss.r, for example.)
  ...
  this.executeMethodURL = function(url, contentParam, requestType, callback, hasResult, accept) {
    ...
    var request = getXmlHttpObjectNew1();
...
/*
 * Returns the host as given in the connectionInfo JSON ObjectNew1 if it exists, otherwise
 * returns the host of the current URL.
 * @param connectionInfo JSON ObjectNew1 containing connection information
 */
...
/*
 * Returns the port as given in the connectionInfo JSON ObjectNew1 if it exists, otherwise
...
function getXmlHttpObjectNew1()
{
  if (typeof(XMLHttpRequest)  === "undefined")
  {
    XMLHttpRequest = function() {
      try 
      { 
        return new ActiveXObjectNew1("Msxml2.XMLHTTP.6.0");
      }
      catch(e) {}
      try
      {
        return new ActiveXObjectNew1("Msxml2.XMLHTTP.3.0");
      }
      catch(e) {}
      try
      {
